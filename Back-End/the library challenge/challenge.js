...
function affordableBooks(budget) {
  ...
  for (let i = 0; i < books.length; i++) {
    if (books[i].price <= budget) {
      affBudget[affBudget.length] = books[i];
    }
  }
  return affBudget;
}
let budget = 3;
let affBudget = affordableBooks(budget);
console.log(affBudget);

for (let i = 0; i < affBudget.length; i++) {
  let book = affBudget[i];
...
function findBookByGenre(genre) {
  ...
  if (newGr) {
    genreBook[genreBook.length] = books[i];
  }
  return genreBook;
}
let elg = "Fiction";
let funGenre = findBookByGenre(elg);

for (let i = 0; i < funGenre.length; i++) {
  let book = funGenre[i];
